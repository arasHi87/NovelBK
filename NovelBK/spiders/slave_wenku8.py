...
class Wenku8SlaveSpider(RedisSpider):
    ...
    #     b_name = cc.convert(response.xpath('//*[@id="title"]/text()').get())
    ...
    #     item['index'][b_name]['author'] = cc.convert(response.xpath('//*[@id="info"]/text()').get().replace('作者：', ''))
    ...
    #             temp = [cc.convert(normalize('NFKD', ele.xpath('text()').get()))]
    #         else:
    #             if ele.xpath('a/text()').get():
    #                 c_name = cc.convert(normalize('NFKD', ele.xpath('a/text()').get()))
    #                 temp.append(c_name)
    ...
    #         c_name = cc.convert(x.xpath('text()').get())
    ...
    #     content = cc.convert(BeautifulSoup(response.xpath('//*[@id="content"]').get().strip(), "lxml").text)
    #     title = cc.convert(normalize('NFKD', response.xpath('//*[@id="title"]/text()').get()))
    #     v_name = cc.convert("".join(title.rsplit(response.meta['c_name'], 1)).strip())
    #     path = os.path.join(loc, self.settings.get('TEMP_STORE_PATH') 'wenku8', response.meta['b_name'], 'txt', v_name)
